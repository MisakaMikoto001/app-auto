...
class FirstLoginBusiness:
    ...
    def first_login_process_with_wechat(self):
        ...
        # 4. 检查弹窗是否显示
        if self.popup_login_wechat_business.is_popup_displayed():
    ...
    def first_login_process_with_flash_test(self,phone, code):
        """首次手机登录完整流程"""
        # 1. 启动APP并同意协议进入
        self.start_page_business.start_page_get_in()
        ...
        # 4. 检查弹窗是否显示
        if self.popup_login_tel_business.is_popup_displayed():
